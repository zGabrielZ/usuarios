...
import br.com.gabrielferreira.usuario.mapper.dto.GeneroDTOMapper;
...
public class GeneroController {
    ...
    private final GeneroDTOMapper generoDTOMapper;

    @GetMapping
    public ResponseEntity<List<GeneroResponseDTO>> buscarGeneros(){
        List<GeneroDomain> generoDomains = generoService.buscarGeneros();
        return ResponseEntity.ok().body(generoDTOMapper.toGenerosDtos(generoDomains));
    }
    ...
    public ResponseEntity<GeneroResponseDTO> buscarGeneroPorId(@PathVariable Long id){
        ...
        return ResponseEntity.ok().body(generoDTOMapper.toGeneroDto(generoDomain));
    }
